...
This document tracks remaining work after the v0.3.0 release. It reconciles the original v0.3.0 spec with GitHub issues and identifies what was completed vs what remains.

---

## v0.3.0 Completed Work

The following items from the original spec were completed:

| Item | Status | Notes |
|------|--------|-------|
| Remove duplicate `launch()` function (#49) | ✅ Done | Only one definition in instance.py |
| Fix `exclude_terminated` parameter | ✅ Done | Now uses AWS API filters |
| Add `--key` option to connect command | ✅ Done | `-k` / `--key` available |
| Consolidate `get_launch_template_id()` | ✅ Done | Only in utils.py now |
| Python 3.10+ upgrade | ✅ Done | Supports 3.10, 3.11, 3.12 |
| Poetry to uv migration | ✅ Done | uv.lock present |
| Replace flake8/black with Ruff | ✅ Done | Ruff configured |
| Add GitHub Actions CI/CD (#24) | ✅ Done | test.yml, publish.yml |
| Error handling improvements (#50) | ✅ Done | exceptions.py, validation.py |
| Bounds checking and validation | ✅ Done | validation.py utilities |
| NoCredentialsError handling (#6) | ✅ Done | Caught in key modules |
| Test coverage >80% | ✅ Done | 238 tests, 85%+ coverage |

### GitHub Issues to Close

The following issues are resolved and should be closed:

- **#49**: Duplicate function definitions - FIXED
- **#50**: Error handling and bounds checking - FIXED
- **#24**: GitHub action tests - FIXED
- **#6**: Error if no credentials - FIXED (NoCredentialsError handled)
- **#1**: Initial functionality - OUTDATED (close as stale)
...
**Acceptance Criteria:**
- [ ] `read_config()` uses the provided parameter
- [ ] Test added to verify custom config paths work
...
**Acceptance Criteria:**
- [ ] Command provides helpful error if required options missing
- [ ] No unhandled exceptions from AWS SDK
...
**Acceptance Criteria:**
- [ ] Default uses `accept-new`
- [ ] Optional `--no-strict-host-key` flag for legacy behavior
...
**GitHub Issue:** #51
**Status:** Partially done (read_config bug remains)

Additional work:
- [ ] Fix read_config parameter bug
...
1. Fix `read_config()` parameter bug (#51)
2. Fix `create-snapshot` input validation (#15)
3. Fix SSH StrictHostKeyChecking (#54)
4. Upload to PyPI (#18)
...
|-------|-------|--------|--------|
| #54 | Security practices | Open | v0.4.0 |
| #53 | Dependency injection | Open | v0.4.x |
| #52 | Type hints | Open | v0.4.x |
| #51 | Configuration management | Open | v0.4.0 |
...
| #15 | create-snapshot errors | Open | v0.4.0 |
