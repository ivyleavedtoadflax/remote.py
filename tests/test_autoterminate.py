"""Tests for CloudWatch-based auto-termination functionality."""
...
from remote.autoterminate import (
    _create_auto_terminate_alarm,
    _delete_auto_terminate_alarm,
    _get_existing_alarm,
    app,
)
...
class TestGetExistingAlarm:
    ...
    def test_should_return_alarm_when_exists(self, mocker):
        ...
        mock_cloudwatch = mocker.patch("remote.autoterminate.get_cloudwatch_client")
        mock_cloudwatch.return_value.describe_alarms.return_value = {
            "MetricAlarms": [
                {
                    "AlarmName": "remotepy-autoterminate-i-123",
                    "StateValue": "OK",
                    "Threshold": 5.0,
                }
            ]
        ...
        result = _get_existing_alarm("remotepy-autoterminate-i-123")

        assert result is not None
        assert result["AlarmName"] == "remotepy-autoterminate-i-123"
        assert result["StateValue"] == "OK"
        mock_cloudwatch.return_value.describe_alarms.assert_called_once_with(
            AlarmNames=["remotepy-autoterminate-i-123"],
...
class TestCreateAutoTerminateAlarm:
    """Tests for _create_auto_terminate_alarm helper function."""

    def test_should_create_alarm_with_correct_parameters(self, mocker):
        """Should call put_metric_alarm with correct parameters."""
        mock_cloudwatch = mocker.patch("remote.autoterminate.get_cloudwatch_client")
        mocker.patch("remote.autoterminate.get_current_region", return_value="us-east-1")
        ...
        assert call_kwargs["AlarmName"] == "remotepy-autoterminate-i-0123456789abcdef0"
        ...
        assert call_kwargs["AlarmActions"] == ["arn:aws:automate:us-east-1:ec2:terminate"]
...
class TestDeleteAutoTerminateAlarm:
    """Tests for _delete_auto_terminate_alarm helper function."""

    def test_should_delete_alarm_when_exists(self, mocker):
        """Should delete alarm and return True when alarm exists."""
        mock_cloudwatch = mocker.patch("remote.autoterminate.get_cloudwatch_client")
        mock_cloudwatch.return_value.describe_alarms.return_value = {
            "MetricAlarms": [{"AlarmName": "remotepy-autoterminate-i-123"}]
        }

        result = _delete_auto_terminate_alarm("i-123")
...
class TestEnableCommand:
    """Tests for the 'auto-terminate enable' command."""

    def test_should_create_alarm_with_default_parameters(self, mocker):
        """Should create alarm with default threshold and duration."""
        mocker.patch(
            "remote.autoterminate.resolve_instance_or_exit",
            return_value=("test-instance", "i-0123456789abcdef0"),
        )
        mocker.patch("remote.autoterminate._get_existing_alarm", return_value=None)
        mock_create = mocker.patch("remote.autoterminate._create_auto_terminate_alarm")

        result = runner.invoke(app, ["enable", "--yes"])

        assert result.exit_code == 0
        mock_create.assert_called_once_with("i-0123456789abcdef0", "test-instance", 5, 30)
        assert "Enabled auto-termination" in result.stdout
    ...
    def test_should_update_alarm_when_already_exists(self, mocker):
        ...
        mocker.patch(
            "remote.autoterminate._get_existing_alarm",
            return_value={"AlarmName": "remotepy-autoterminate-i-123"},
        )
        mock_create = mocker.patch("remote.autoterminate._create_auto_terminate_alarm")
        ...
        assert "Updated auto-termination" in result.stdout
    ...
    def test_should_prompt_for_confirmation_without_yes_flag(self, mocker):
        ...
        mocker.patch("remote.autoterminate._get_existing_alarm", return_value=None)
        mocker.patch("remote.autoterminate._create_auto_terminate_alarm")
        mock_confirm = mocker.patch("remote.autoterminate.confirm_action", return_value=False)
...
class TestDisableCommand:
    """Tests for the 'auto-terminate disable' command."""
    ...
    def test_should_delete_alarm_when_exists(self, mocker):
        ...
        mock_delete = mocker.patch(
            "remote.autoterminate._delete_auto_terminate_alarm", return_value=True
        )
        ...
        assert "Disabled auto-termination" in result.stdout

    def test_should_warn_when_alarm_not_enabled(self, mocker):
        """Should show warning when auto-termination is not enabled."""
        mocker.patch(
            "remote.autoterminate.resolve_instance_or_exit",
            return_value=("test-instance", "i-123"),
        )
...
class TestStatusCommand:
    """Tests for the 'auto-terminate status' command."""

    def test_should_show_status_when_enabled(self, mocker):
        """Should display alarm status when auto-termination is enabled."""
        mocker.patch(
            "remote.autoterminate.resolve_instance_or_exit",
            return_value=("test-instance", "i-0123456789abcdef0"),
        )
        mocker.patch(
            "remote.autoterminate._get_existing_alarm",
            return_value={
                "AlarmName": "remotepy-autoterminate-i-0123456789abcdef0",
                "StateValue": "OK",
                "Threshold": 5.0,
                "Period": 300,
                "EvaluationPeriods": 6,
